...
const INPUT_PRICE_PER_1M = 0.10; // Placeholder
const OUTPUT_PRICE_PER_1M = 0.40; // Placeholder
...
export const generateMenuFromDump = async (
        ...
        if (!import.meta.env.VITE_GLM_API_KEY) {
                const msg = "[CRITICAL] VITE_GLM_API_KEY is missing from environment variables.";
                onLog(msg);
                throw new Error(msg);
        }
  ...
  `;
        ...
        try {
                ...
                const verboseInterval = setInterval(() => {
                        ...
                        if (elapsed > 2000 && elapsed < 4000) onLog("[SYSTEM] Handshaking with remote cluster...");
                        if (elapsed > 5000 && elapsed < 7000) onLog("[PROCESS] Allocating tensor buffers for context window...");
                        if (elapsed > 8000 && elapsed < 10000) onLog("[COMPUTE] Resolving attention heads (MoE layer)...");
                        if (elapsed > 12000 && elapsed < 14000) onLog("[WAIT] Processing complex input vectors...");
                        if (elapsed > 15000 && elapsed < 17000) onLog("[SYSTEM] Connection stable. Awaiting token generation...");
                        if (elapsed > 20000 && elapsed < 22000) onLog("[WARN] High latency detected. Rerouting packets...");
                }, 1000);
