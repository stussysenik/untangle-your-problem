...
// --- NIM configuration (single source of truth for the generation core) ---
...
export const SYSTEM_PROMPT = `You are a master delegator and executive assistant.
...
/** Optional progress hook — the Temporal activity passes `heartbeat`; tests pass a no-op. */
export type ProgressFn = (note: string) => void;
const noop: ProgressFn = () => {};

export function resolveNimKey(): string {
  return process.env.NIM_API_KEY ?? process.env.NVIDIA_NIM_API_KEY ?? '';
}

/**
 * The generation core: a brain dump in, a validated Menu out.
 * Pure of any Temporal/runtime coupling so it can be exercised in isolation.
 */
export async function untangle(text: string, onProgress: ProgressFn = noop): Promise<Menu> {
  const nimKey = resolveNimKey();
  if (!nimKey) throw new Error('NIM_API_KEY not configured');

  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), REQUEST_TIMEOUT_MS);

  onProgress('Starting NIM request');
  ...
  try {
    ...
    onProgress('NIM response received, parsing');
