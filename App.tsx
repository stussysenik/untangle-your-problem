...
import { generateMenuFromDump } from './services/geminiService';
...
export default function App() {
        ...
        return (
            <div className="flex flex-col h-full justify-center max-w-lg mx-auto w-full">
                <div className="mb-8 border-b border-black pb-2 flex justify-between items-baseline">
                    <span className="uppercase tracking-widest font-bold font-mono text-xs">Processing</span>
                    {/* Animated Dots */}
                    <span className="font-mono text-xs w-6">{loadingDots}</span>
                </div>
                ...
                <div className="flex flex-col gap-8 lg:gap-10">
                    {menuItems.map((item, index) => {
                        ...
                        return (
                            ...
                            >
                                <div className="flex flex-row gap-4 lg:gap-6 items-baseline">
                                    {/* MANIFESTO NUMBERING - MINIMALIST */}
                                    <div className="flex flex-col items-start flex-shrink-0 w-12 lg:w-16">
                                        <span className="font-mono text-3xl lg:text-4xl font-light tracking-tighter leading-none text-black">
                                    ...
                                    <div className="flex-1 space-y-1.5 pt-0">
                                        <div className="flex flex-col lg:flex-row lg:justify-between lg:items-baseline gap-1 lg:gap-2">
                                            {/* DISH NAME - The primary highlight target */}
                                            <h3 className={`font-mono font-bold text-base lg:text-lg uppercase tracking-tight transition-colors duration-200 inline-block ${isActive ? 'bg-insight text-black' : 'text-black'}`}>
                                                {item.dishName}
                                            </h3>
                                            
                                            <span className="font-mono text-[10px] text-gray-400 whitespace-nowrap bg-gray-50 px-2 py-0.5 rounded self-start lg:self-auto">
                                                {item.quantity}
                                            </span>
                                        </div>
                                        {/* Advice - Scaled down for "Menu" feel */}
                                        <p className="font-serif text-sm lg:text-base leading-relaxed text-gray-700 max-w-2xl">
                        ...
                        return (
                            <span 
                                ...
                                isActive 
                                    ...
                                    : isHighlighted 
                                    ? 'text-black font-semibold border-b border-gray-200 decoration-clone' 
                                    : 'text-gray-400'
                                }`}
  ...
  return (
    ...
    <div className="min-h-screen w-full bg-white text-black selection:bg-insight selection:text-black flex flex-col lg:flex-row lg:h-screen lg:overflow-hidden">
      ...
      <div className="left-panel-container w-full lg:w-1/2 flex-shrink-0 h-auto lg:h-full p-6 lg:p-16 border-b lg:border-b-0 lg:border-r border-gray-100 bg-white z-20 lg:overflow-hidden flex flex-col">
        {renderLeftPanel()}
      </div>

      {/* RIGHT PANEL */}
      {/* Mobile order: Bottom. Needs min-height for typing comfortably on mobile */}
      <div className="right-panel-container w-full lg:w-1/2 flex-shrink-0 min-h-[50vh] lg:h-full bg-gray-50 p-6 lg:p-16 relative z-10 lg:overflow-hidden flex flex-col">
         {renderRightPanel()}
      </div>
